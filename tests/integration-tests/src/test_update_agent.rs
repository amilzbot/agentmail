...
fn test_update_agent_success() {
    ...
    assert!(account.is_some(), "Agent registry account should still exist");
    ...
    assert!(registry.updated_at > registry.created_at, "updated_at should be newer than created_at");
}
...
fn test_update_agent_preserves_other_fields() {
    ...
    // Get original data
    let original_account = context.get_account(&agent_registry_pda.into()).unwrap();
    let original_registry = AgentRegistryAccount::try_from_account_data(&original_account.data).unwrap();
    ...
    let updated_account = context.get_account(&agent_registry_pda.into()).unwrap();
    let updated_registry = AgentRegistryAccount::try_from_account_data(&updated_account.data).unwrap();
